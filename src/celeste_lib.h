...
BumpAllocator make_bump_allocator(size_t size)
{
    ...
    {
        SM_ASSERT(false, "Failed to allocate memory for bump allocator.");
    }
...
char* bump_alloc(BumpAllocator* bumpAllocator, size_t size)
{
   ...
   size_t allignedSize = (size + 7) & ~ 7; // first 4 bits are set to 0
   ...
   {
       SM_ASSERT(false, "Bump allocator out of memory. (full)");
   }
